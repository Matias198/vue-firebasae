         ...
         createUserWithEmailAndPassword(auth, email.value, password.value)
             .then((userCredential) => {
                 ...
                 subirImagenFirebaseStorage().then((downloadURL) => {
                     const base64Image = downloadURL;
                     setDoc(usuarioDoc, { username, base64Image }) // Guardar el nombre de usuario en Cloud Firestore
                         .then(() => {
                     ...
                     Swal.fire({
                         icon: 'error',
                         title: 'Error al crear usuario',
                         text: 'No se pudo subir la imagen a Firestore Storage',
                         confirmButtonText: 'Aceptar'
                     })
                     loader.hide();
                 })
...
<template> 
    <div class="flex justify-center p-4">
        <button id="buttonRegister" data-modal-toggle="modal-register" data-modal-target="modal-register" type="button"
            ...
            <svg class="w-3.5 h-3.5 me-2 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
